...
class ZhihuUserSpider(CrawlSpider):
    ...
    def parse_zhihu(self, response):
        ...
        # 得到followees的链接
        followee_link = sel.xpath('//div[@class="zu-main-sidebar"]/div/a/@href').extract()[0]
        headers = self.headers;
        headers['Referer'] = response.url
        yield scrapy.FormRequest(host+followee_link, headers = headers, cookies=self.cookies, callback=self.parse_followees)
        
        #得到followers的链接
        follower_link = sel.xpath('//div[@class="zu-main-sidebar"]/div/a/@href').extract()[1]
        headers = self.headers;
        headers['Referer'] = response.url
        yield scrapy.FormRequest(host+follower_link, headers = headers, cookies=self.cookies, callback=self.parse_followers)
